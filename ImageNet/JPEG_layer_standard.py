...
class JPEG_layer(nn.Module):
    ...
    def construct(self, opt):
        ...
        if os.path.isfile(opt.sensitvity_dir + "Y" + opt.model + ".npy") and opt.initial_Q_w_sensitivity:
            Y_sens  = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Y" + opt.model + ".npy")))
            Cb_sens = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Cb"+ opt.model + ".npy")))
            Cr_sens = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Cr"+ opt.model + ".npy")))
            ...
            print("JPEG layer ==> load sensitivity and Normalize it by ... {:.4f}".format(factor))
        
            # if self.q_min is not None:
            #     min_Y = torch.min(Y_sens)
            #     max_Y = torch.max(Y_sens)
            #     min_CbCr = torch.min(CbCr_sens)
            #     max_CbCr = torch.max(CbCr_sens)
            #     self.Y_sens = ((Y_sens - min_Y)/(max_Y-min_Y)) * (self.q_max - self.q_min) + self.q_min
            #     self.CbCr_sens = ((CbCr_sens - min_CbCr)/(max_CbCr-min_CbCr)) * (self.q_max - self.q_min) + self.q_min
            #     print("JPEG layer ==> load sensitivity and Normalize it by ... {:.4f} <==> min_q {} max_q {}".format(factor,  torch.min(self.Y_sens),  torch.max(self.Y_sens)))
            ...
            print("JPEG layer ==> load absloute average and Normalize it ")
        
        else:
            print("No senstivity generated ...") 
            # exit(0)       
