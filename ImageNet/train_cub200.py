...
def load_model_imagenet(name, num_classes, pretrained=False, **kwargs):
    if name=="resnet18":
...
def main(args):
    ...
    data_loader, data_loader_test = load_dataset(name="CUB200", mean=mean_datatloader, std=std_datatloader, batch_size=args.batch_size)
    num_classes = len(data_loader.dataset.class_names)
    
    print("Creating model")
    # underlying_model = torchvision.models.get_model(args.model, weights=args.weights, num_classes=num_classes)
    underlying_model =  load_model_imagenet(args.model, num_classes=num_classes, pretrained=False)
    if args.JPEG_enable:
        jpeg_layer = JPEG_layer(img_shape=(224, 224, 3), mean=mean, std=std)
    ...
    if  args.lr_scheduler == "multisteplr":
        main_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100, 150], gamma=0.1)
    ...
    start_time = time.time()
    log_fname = args.output_dir+'log.txt'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(log_fname, 'a+') as log:
            log.write(str(epoch) + "\t" +str(tp1_accuracy)+'\t'+ str(best_accuracy) + '\n')
    ...
    exp_txt.write(args.model +"\t"+ str(epoch) + "\t" + str(best_accuracy) + "\t" + total_time_str + "\n") # Write some text
