...
#             echo  ${dataset}
#             CUDA_VISIBLE_DEVICES=0,1 python3.8 -m torch.distributed.launch --master_port=25671 --nnodes=1 --nproc_per_node=2 main.py \
#                                                     --model ${teacher}  --warmup_epochs 100 --epochs 600 \
#                                                     --batch_size 128 --lr 2e-3 --update_freq 1 --use_amp true \
        ...
        for q_max in 5; do
            for JEPG_alpha in 5; do
                ...
                while (( $(awk -v start="$start_1" -v end="$end_1" 'BEGIN { if (start <= end) print 1; else print 0 }') )); do
                    ...
                    # CUDA_VISIBLE_DEVICES=0,1 python3.8 -m torch.distributed.launch --master_port=25671 --nnodes=1 --nproc_per_node=2 main_JPEG.py \
                    #                                         --model ${teacher}  --warmup_epochs 100 --epochs 600 \
                    #                                         --batch_size 128 --lr 2e-3 --update_freq 1 --use_amp true \
                    #                                         --data_set ${dataset} --save_ckpt_freq 100 \
                    ...
                    CUDA_VISIBLE_DEVICES=4,7 python3.8 -m torch.distributed.launch --master_port=25670 --nnodes=1 --nproc_per_node=2 main_JPEG.py \
                                                            --model ${teacher}  --warmup_epochs 100 --epochs 600 \
                                                            --batch_size 256 --lr 2e-3 --update_freq 1 --use_amp true \
                                                            --data_set ${dataset} --save_ckpt_freq 100 \
                                                            --output_dir ./save/JPEG/${dataset}/SGD/${teacher}_JPEG_lr_${JEPG_learning_rate}_alpha_${JEPG_alpha}_q_max_${q_max}_numBits_${numBits}_${trial} \
                                                            --data_path /home/ahamsala/PROJECT_AH/JPEG_DNN/data/ \
