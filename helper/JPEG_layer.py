...
class JPEG_layer(nn.Module):
    ...
    def construct(self, opt):
        ...
        print(f"Loading the correct sensitivity from {opt.sensitvity_dir} Y_ {opt.model} .npy ")
    ...
    def __init__(self, opt, img_shape, mean=[0.5071, 0.4867, 0.4408], std=[0.2675, 0.2565, 0.2761], analysis=False):
        ...
        if os.path.isfile(opt.sensitvity_dir + "Y_" + opt.model + ".npy") and opt.initial_Q_w_sensitivity:
        ...
        if not self.zero_dc:
           ...
           if torch.cuda.is_available():
                self.lum_qtable = self.lum_qtable.cuda()
                self.chrom_qtable = self.chrom_qtable.cuda()
        
        if opt.JPEG_alpha_trainable and not self.zero_dc:
