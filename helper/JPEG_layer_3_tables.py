...
class JPEG_layer(nn.Module):
    ...
    def __init__(self, opt, img_shape, mean=[0.5071, 0.4867, 0.4408], std=[0.2675, 0.2565, 0.2761]):
        ...
        self.alpha_chrom = torch.ones((1, 2, 1, opt.block_size, opt.block_size, 1), dtype=torch.float32)
    ...
    def forward(self, input_RGB):
        ...
        estimated_reconstructed_space_chrom  = torch.sum(F.softmax(-self.alpha_chrom * distortion_MSE_mask_chrom , dim=-1) * iq_chrom, -1)
