...
class JPEG_layer(nn.Module):
    ...
    def construct(self, opt):
        ...
        if os.path.isfile(opt.sensitvity_dir + "Y_" + opt.model + ".npy"):
            Y_sens  = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Y_" + opt.model + ".npy")))
            Cb_sens = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Cb_"+ opt.model + ".npy")))
            Cr_sens = inverse_zigzag(torch.from_numpy(np.load(opt.sensitvity_dir + "Cr_"+ opt.model + ".npy")))
            
            Y_sens = 1 / Y_sens 
            CbCr_sens =  2 / (Cb_sens + Cr_sens)
            _    , factor = normalize(Y_sens   , 0)
            factor = factor / self.q_max
            self.Y_sens    , _   = normalize(Y_sens   , factor)
            self.CbCr_sens , _   = normalize(CbCr_sens, factor)
            # print(self.Y_sens)
            print("JPEG layer ==> load sensitivity and Normalize it by ... {}".format(factor))
        


        if os.path.isfile(opt.dir_initial_std):
            tables_std = torch.load(opt.dir_initial_std)
            self.std_Y  = tables_std["std_Y"].view(1,1,1,self.block_size, self.block_size)
            self.std_CbCr = tables_std["std_CbCr"].view(1,1,1,self.block_size, self.block_size)
        
        if os.path.isfile(opt.dir_initial_mean):
            tables_mean = torch.load(opt.dir_initial_mean)
            self.mean_Y  = tables_mean["mean_Y"].view(1,1,1,self.block_size, self.block_size)
            self.mean_CbCr = tables_mean["mean_CbCr"].view(1,1,1,self.block_size, self.block_size)
        
        if os.path.isfile(opt.dir_initial_abs_avg):
            tables_abs_sum = torch.load(opt.dir_initial_abs_avg)
            self.abs_avg_Y  = tables_abs_sum["abs_avg_Y"].view(1,1,1,self.block_size, self.block_size) 
            self.abs_avg_CbCr = tables_abs_sum["abs_avg_CbCr"].view(1,1,1,self.block_size, self.block_size)
        ...
        self.Y_level = second_max_fn(self.std_Y)
        self.CbCr_level = second_max_fn(self.std_CbCr)
        ...
        if torch.cuda.is_available():
            ...
            self.Y_level = self.Y_level.cuda()
            self.CbCr_level = self.CbCr_level.cuda()
            ...
            self.mean_Y = self.mean_Y.cuda()
            self.std_Y = self.std_Y.cuda()
            self.mean_CbCr = self.mean_CbCr.cuda()
            self.std_CbCr = self.std_CbCr.cuda()
    ...
    def __init__(self, opt, img_shape, mean=[0.5071, 0.4867, 0.4408], std=[0.2675, 0.2565, 0.2761], analysis=False):
        ...
        self.hardness_Y  = opt.hardness * torch.ones((1, 1, 1, opt.block_size, opt.block_size, 1), dtype=torch.float32)
        self.hardness_CbCr  = opt.hardness * torch.ones((1, 1, 1, opt.block_size, opt.block_size, 1), dtype=torch.float32)
        
        self.construct(opt)

        if self.jpeg_layers > 1:
            self.lum_qtable = torch.ones((opt.jpeg_layers, 1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.chrom_qtable = torch.ones((opt.jpeg_layers, 1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.alpha_lum = torch.ones((opt.jpeg_layers, 1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.alpha_chrom = torch.ones((opt.jpeg_layers,1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
        else:
            self.lum_qtable = torch.ones((1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.chrom_qtable = torch.ones((1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.alpha_lum = torch.ones((1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
            self.alpha_chrom = torch.ones((1, 1, 1, self.block_size, self.block_size, 1), dtype=torch.float32)
        ...
        if os.path.isfile(opt.dir_initial_abs_avg) and opt.initial_Q_abs_avg:
            print("JPEG layer ==> ** Q table ** is initialized by the absolute average ... ")
            self.lum_qtable = 2 * self.abs_avg_Y / (2**(self.num_bit - 1)) ** (0.5)
            self.chrom_qtable = 2 * self.abs_avg_CbCr / (2**(self.num_bit - 1)) ** (0.5)
            self.lum_qtable = self.lum_qtable.view((1, 1, 1, self.block_size, self.block_size, 1))
            self.chrom_qtable = self.chrom_qtable.view((1, 1, 1, self.block_size, self.block_size, 1))
        elif os.path.isfile(opt.sensitvity_dir + "Y_" + opt.model + ".npy") and opt.initial_Q_w_sensitivity:
            print("JPEG layer ==> ** Q table ** is initialized by Sensitivity using inverse normalizarion method ... ")
            self.lum_qtable = self.lum_qtable * self.Y_sens.view(1,1,1,self.block_size, self.block_size, 1)
            self.chrom_qtable = self.chrom_qtable * self.CbCr_sens.view(1,1,1,self.block_size, self.block_size, 1)
        else:
    ...
    def forward_jpeg_layer(self, input_RGB, analysis=False):
        ...
        # input_lum = (input_lum - input_DCT_block_batch[:, 0, ...].mean(dim=(0,1)))/input_DCT_block_batch[:, 0, ...].std(dim=(0,1))
        # input_chrom = (input_chrom - input_DCT_block_batch[:, 1:3, ...].mean(dim=(0,1)))/input_DCT_block_batch[:, 1:3, ...].std(dim=(0,1))
        ...
        cmpf_mask_chrom = F.softmax(-self.alpha_chrom.expand(-1, 2, -1, -1, -1, 1) * distortion_MSE_mask_chrom , dim=-1) # shape: [bs,ch,B,size,size,5] 
