...
class JPEG_layer(nn.Module):
    ...
    def construct(self, opt):
        ...
        self.p_quantile = opt.p_quantile
        self.num_non_zero_q = opt.num_non_zero_q

        tables = torch.load(opt.dir_initial)
        self.std_Y  = tables["std_Y"]
        self.std_CbCr = tables["std_CbCr"]
        self.std_width = opt.std_width
        self.num_bits_Y = torch.round(torch.log2(2*self.std_width * self.std_Y + 2))
        self.num_bits_CbCr = torch.round(torch.log2(2*self.std_width * self.std_CbCr + 2))
        
        # Gerneral low leve and high level when we set num_bit ()
        self.low_level  = -2**(self.num_bit-1) + 1
        self.high_level =  2**(self.num_bit-1)

        self.low_level_Y  =  (-2**(self.num_bits_Y + 1) + 1).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 
        self.high_level_Y =  ( 2**(self.num_bits_Y)).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 

        self.low_level_CbCr  =  (-2**(self.num_bits_CbCr + 1) + 1).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 
        self.high_level_CbCr =  ( 2**(self.num_bits_CbCr)).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 

        self.std_Y_clamp  = (self.std_Y * self.std_width).view(1, 1, 1, self.block_size, self.block_size, 1) 
        self.std_CbCr_clamp =  (self.std_CbCr * self.std_width).view(1, 1, 1, self.block_size, self.block_size, 1) 
        
        
        self.num_non_zero_q_on_1_side = math.floor(self.num_non_zero_q/2)
        self.q_idx = torch.arange(0, self.num_non_zero_q)

        self.alp_range = torch.arange(0, 260, 2)
        self.p_quantile_range = torch.arange(0.1, 1.05, 0.05)
        self.q_step = 1.0 # If you set it to "None", the quantization step will be calcuated
        ...
        if torch.cuda.is_available():
            ...
            self.std_Y_clamp = self.std_Y_clamp.cuda()
            self.std_CbCr_clamp = self.std_CbCr_clamp.cuda()
    ...
    def __init__(self, opt, img_shape, mean=[0.5071, 0.4867, 0.4408], std=[0.2675, 0.2565, 0.2761], analysis=False):
        ...
        self.lum_qtable = torch.ones((1, 1, 1, opt.block_size, opt.block_size, 1), dtype=torch.float32)
        self.chrom_qtable = torch.ones((1, 1, 1, opt.block_size, opt.block_size, 1), dtype=torch.float32)
        ...
        nn.init.constant_(self.alpha_lum, opt.JEPG_alpha)
        nn.init.constant_(self.alpha_chrom, opt.JEPG_alpha)
    ...
    def forward(self, input_RGB, analysis=False):
        ...
        if self.FLAG_clamp_w_std:
            # Clampping based on the standard deviation
            idx_lum   =  torch.clamp(idx_lum   - self.num_non_zero_q_on_1_side, min=-1 * self.std_Y_clamp, max=self.std_Y_clamp - self.num_non_zero_q)
            idx_chrom =  torch.clamp(idx_chrom - self.num_non_zero_q_on_1_side, min=-1 * self.std_CbCr_clamp, max=self.std_CbCr_clamp - self.num_non_zero_q)
            # print(idx_lum_[:,:,:,0,0,:].max().item(), idx_lum[:,:,:,0,0,:].max().item(), (self.std_Y_clamp - self.num_non_zero_q)[:,:,:,0,0,0].item())
            # print(idx_lum_[:,:,:,0,0,:].min().item(), idx_lum[:,:,:,0,0,:].min().item(), (-1 * self.std_Y_clamp)[:,:,:,0,0,0].item())
            ...
            # print(idx_lum_[:,:,:,0,0,:].max().item(), idx_lum[:,:,:,0,0,:].max().item(), (self.high_level_Y - self.num_non_zero_q)[:,:,:,0,0,0].item())
            # print(idx_lum_[:,:,:,0,0,:].min().item(), idx_lum[:,:,:,0,0,:].min().item(), self.low_level_Y[:,:,:,0,0,0].item())
    ...
    def initialize_alpha(self, train_loader):
        Freq_idx = torch.zeros(self.block_size, self.block_size, 3,  len(train_loader.dataset)*self.num_block, dtype=torch.float32)
        if torch.cuda.is_available():
            self = self.cuda()
            Freq_idx = Freq_idx.cuda()
            
        for idx, (input, target) in enumerate(train_loader):
            if torch.cuda.is_available():
                input = input.cuda()
            _, input_DCT_block_batch, _  = self.forward(input, analysis=True)
            input_DCT_block_batch = input_DCT_block_batch.squeeze(-1)
            
            st = idx * input.size(0) * self.num_block
            ed = (idx + 1) * input.size(0) * self.num_block
            
            for c in range(3):
                for i in range(self.block_size):
                    for j in range(self.block_size):
                        Freq_idx[i, j, c, st:ed] = input_DCT_block_batch[: , c, :, i, j].flatten()
        
        self.std_Y = torch.zeros((self.block_size, self.block_size))
        for i in range(self.block_size):
            for j in range(self.block_size):
                 self.std_Y[i,j] = Freq_idx[i, j, Channel.Y_Channel.value].std()

        self.std_CbCr = torch.zeros((self.block_size, self.block_size))
        for i in range(self.block_size):
            for j in range(self.block_size):
                self.std_CbCr[i,j] = Freq_idx[i, j, Channel.Cb_channel.value: (Channel.Cr_Channel.value + 1)].view( 8, 8, -1).std()

        self.num_bits_Y = torch.round(torch.log2(2*self.std_width * self.std_Y + 2))
        self.num_bits_CbCr = torch.round(torch.log2(2*self.std_width * self.std_CbCr + 2))

        self.low_level_Y  =  (-2**(self.num_bits_Y-1) + 1).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 
        self.high_level_Y =  ( 2**(self.num_bits_Y-1)).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 

        self.low_level_CbCr  =  (-2**(self.num_bits_CbCr - 1) + 1).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 
        self.high_level_CbCr =  ( 2**(self.num_bits_CbCr - 1)).view(1, 1, 1, self.block_size, self.block_size, 1) # Batch, Channel, B, B, 1 
        ...
        plot_manager_Y = PlotManager(Channel.Y_Channel.name, self.std_width)
        plot_manager_CbCr = PlotManager(Channel.CbCr_Channel.name, self.std_width)


        self.alp_range = torch.arange(1, 110, 10)
        # self.curvature_theshould = torch.arange(0.1, 1.1, 0.1)
        self.curvature_theshould = torch.arange(0.1, 0.7, 0.1)
        # self.curvature_theshould = torch.tensor([0.2])
        # self.alp_range = torch.arange(50, 150, 50)
        p_quantile = 0.1

        for i in range(self.block_size):
            for j in range(self.block_size):
                num_bit = self.num_bits_Y[i,j]
                # if self.q_step is None: q_step = (2 * self.std_width * Freq_idx[i, j].std())/(2 ** num_bit - 2)
                Freq_idx_unique_elements, counts = torch.unique(Freq_idx[i, j, Channel.Y_Channel.value], return_counts=True)
                # P_Freq_idx = counts.float() / Freq_idx.size(-1)
                # Freq_idx_unique_elements = torch.linspace(Freq_idx[i, j, Channel.Y_Channel.value].min().item(), 
                #                                           Freq_idx[i, j, Channel.Y_Channel.value].max().item(), 
                #                                           Freq_idx[i, j, Channel.Y_Channel.value].shape[0]).cuda()
                # breakpoint()
                if self.FLAG_clamp_w_std:
                    low_level  = -1 * self.std_Y_clamp[0,0,0,i,j,0]
                    high_level =  1 * self.std_Y_clamp[0,0,0,i,j,0]
                else:
                    low_level = self.low_level_Y[0,0,0,i,j,0]
                    high_level = self.high_level_Y[0,0,0,i,j,0]
                
                k_per_alpha = torch.zeros((len(self.curvature_theshould), len(self.alp_range)), dtype=float)
                for idx_, alpha in enumerate(self.alp_range):
                    function_applied, bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf = \
                        cal_Degree_of_NonLinearity(num_bit, self.lum_qtable[0,0,0,i,j,0], alpha, Freq_idx_unique_elements, 
                                                        low_level, high_level, self.num_non_zero_q)
                    plot_manager_Y.plot_step( i + 8*j, [i, j], bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf, label="{}".format(alpha))
                    

                    # Degree_of_NonLinearity = torch.quantile(function_applied, p_quantile, interpolation='higher')
                    # k_per_alpha[0, idx_] = Degree_of_NonLinearity.item() 
                    # for idx_1, curvature_theshould in enumerate(self.curvature_theshould):
                    #     # breakpoint()
                    #     # indices = torch.nonzero(torch.tensor(bin_Degree_of_NonLinearity_cdf) > curvature_theshould , as_tuple=True)[0][0]
                    #     # p_quantile = Degree_of_NonLinearity_cdf[indices].round(5)
                    #     p_quantile = torch.tensor(curvature_theshould).clone().detach().cuda()
                    #     Degree_of_NonLinearity = torch.quantile(function_applied, p_quantile, interpolation='higher')
                    #     k_per_alpha[idx_1, idx_] = Degree_of_NonLinearity.item()                          
                        
                plot_manager_Y.save_step()
                
                
                # plot_manager_Y.plot_line( i + 8*j , [i, j], self.alp_range, k_per_alpha, self.curvature_theshould)
                # plot_manager_Y.save_line()
                # breakpoint()
                # self.alpha_lum[..., i, j, 0] = self.alp_range[torch.argmax(k_per_alpha)]

                # ---------------------------------------------------------------------------------------------------------------------------------------------------------

                num_bit = self.num_bits_CbCr[i,j]
                # if self.q_step is None: q_step = (2 * self.std_width * Freq_idx[i, j].std())/(2 ** num_bit - 2)
                k_per_alpha = torch.zeros_like(self.alp_range, dtype=float)
                Freq_idx_unique_elements, counts = torch.unique(Freq_idx[i, j, Channel.Cb_channel.value: (Channel.Cr_Channel.value + 1) ], return_counts=True)
                # P_Freq_idx = counts.float() / Freq_idx.size(-1)
                # Freq_idx_unique_elements = torch.linspace(Freq_idx[i, j].min().item(), 
                #                                           Freq_idx[i, j].max().item(), Freq_idx[i, j].shape[0]).cuda()
                if self.FLAG_clamp_w_std:
                    low_level  = -1 * self.std_CbCr_clamp[0,0,0,i,j,0]
                    high_level =  1 * self.std_CbCr_clamp[0,0,0,i,j,0]
                else:
                    low_level = self.low_level_CbCr[0,0,0, i,j, 0]
                    high_level = self.low_level_CbCr[0,0,0, i,j, 0]
                
                for idx_, alpha in enumerate(self.alp_range):
                    function_applied, bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf  = \
                        cal_Degree_of_NonLinearity(num_bit, self.chrom_qtable[0,0,0,i,j,0], alpha, Freq_idx_unique_elements, 
                                                        low_level, high_level, self.num_non_zero_q)
                    plot_manager_CbCr.plot_step( i + 8*j, [i, j], bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf, label="{}".format(alpha))
                    # k_per_alpha[0, idx_] = Degree_of_NonLinearity.item() 
                    # for idx_1, curvature_theshould in enumerate(self.curvature_theshould):
                    #     # breakpoint()
                    #     # If we want to fix the level of curvature 
                    #     # indices = torch.nonzero(torch.tensor(bin_Degree_of_NonLinearity_cdf) > curvature_theshould , as_tuple=True)[0][0]
                    #     # p_quantile = Degree_of_NonLinearity_cdf[indices].round(5)
                ...
                plot_manager_CbCr.save_step()
                # plot_manager_CbCr.save_line()
                
                # self.alpha_chrom[..., i,j, 0] = self.alp_range[torch.argmax(k_per_alpha)]
          
        breakpoint()

def cal_Degree_of_NonLinearity(num_bit, q_step, alpha, Freq_idx_unique_elements, low_level, high_level, num_non_zero_q=5):
    num_non_zero_q_on_1_side = math.floor(num_non_zero_q/2)
    q_idx = torch.arange(0, num_non_zero_q)
    if torch.cuda.is_available():
        q_idx = q_idx.cuda()
    
    if not torch.is_tensor(num_bit):
        num_bit = torch.tensor(num_bit, dtype=torch.float32)
    if not torch.is_tensor(alpha):
        alpha = torch.tensor(alpha, dtype=torch.float32)   
    if not torch.is_tensor(q_step):
        q_step  = torch.tensor(q_step, dtype=torch.float32)    

    Freq_idx_unsqueeze = Freq_idx_unique_elements.unsqueeze(-1)
    Freq_idx_unsqueeze.requires_grad = True

    idx   = torch.round(Freq_idx_unsqueeze / q_step)
    idx   =  torch.clamp(idx - num_non_zero_q_on_1_side, min=low_level, max=high_level - num_non_zero_q)
    idx = idx.expand(-1, num_non_zero_q) + q_idx
    iq = idx * q_step
    distortion_MSE_mask_lum = F.mse_loss(iq, Freq_idx_unsqueeze.expand(-1, num_non_zero_q), reduction='none')
    prob = F.softmax(-alpha * distortion_MSE_mask_lum, dim=1)
    # This is the first derviative for our analyitical function 
    grad_1 = 2 * alpha * (torch.sum(iq**2 * prob, 1) - (torch.pow(torch.sum(iq * prob, 1), 2)))
    grad_1_mean = grad_1.sum()
    grad_1_mean.backward()
    grad_2 = Freq_idx_unsqueeze.grad.squeeze(-1)
    # with torch.no_grad():     
        # Here we measure the Curvature of a graph function
    function_applied = torch.abs(grad_2.float()) / torch.pow(1 + grad_1.float() ** 2, 3/2)
    bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf = ECDF_fn(function_applied)

    
    return function_applied, bin_Degree_of_NonLinearity_cdf, Degree_of_NonLinearity_cdf
    


# Define a class to hold and manage figures
class PlotManager:
    def __init__(self, channel_name, std_width):
        # plot_manager_Y.figures_cdf[0].gca().lines
        self.figures_lines = {}
        self.data_lines = {}
    ...
    def plot_line(self, fig_number, data, x, y, curvature_theshould, **kwargs):
        if fig_number not in self.figures_lines:
            self.figures_lines[fig_number] = plt.figure(fig_number)
            self.data_lines[fig_number] = data

        plt.figure(fig_number)
        for idx, curvature_theshould_ in enumerate(curvature_theshould):
            plt.step(x, y[idx], where='post', **kwargs,  label="{:.3f}".format(curvature_theshould_))

    def plot_step(self, fig_number, data, x, y, label, **kwargs):
        fig_number = fig_number+1000
        if  fig_number not in self.figures_cdf:
            self.figures_cdf[fig_number] = plt.figure(fig_number)
            self.data_cdf[fig_number] = data
    ...
    def save_line(self):
        for key, fig in self.figures_lines.items(): 
            plt.figure(fig.number)
            i, j = self.data_lines[key]
            plt.xlabel(r"$\alpha$")
            plt.ylabel(f"$Q(p)$")
            plt.title('Degree of Non-Linearity ({},{}) ; std {}'.format(i, j, self.std_width))
            plt.legend()
            plt.grid(True)
            plt.savefig("./JPEG_DNN_analysis/DoN/line_DoN_{} ({},{}).png".format(self.channel_name, i, j), dpi=600)
            plt.close()
    

    def save_step(self):
        for key, fig in self.figures_cdf.items():
            plt.figure(fig.number)
            i, j = self.data_cdf[key]
            plt.title('CDF vs. Alpha ({},{}) ; std {}'.format(i, j, self.std_width))
            plt.xlabel('Curvature $k(x)$')
            plt.ylabel('CDF') 
            plt.xlim((0, 1))
            plt.grid(True)
            plt.legend()
            plt.savefig("./JPEG_DNN_analysis/DoN/cdf_line_DoN_{} ({},{}).png".format(self.channel_name, i, j), dpi=600)
            plt.close()


def ECDF_fn(tensor):
    # Sort the random variable tensor
    tensor = tensor.flatten().detach().cpu()
    sorted_data, indices = torch.sort(tensor)
    # Calculate cumulative probabilities
    total_points = len(sorted_data)
    cum_probs = torch.arange(1, total_points + 1) / total_points
    return sorted_data.numpy(), cum_probs.numpy()
