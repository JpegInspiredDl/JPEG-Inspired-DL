...
def train(epoch, train_loader, model, criterion, optimizer, opt):
    ...
    accumulation_steps = opt.accumulation_steps  # Number of micro-batches to accumulate
...
def train_JPEG(epoch, train_loader, model, criterion, optimizer_underlying_model, optimizer_JPEG_Layer, opt):
    """vanilla training"""
    model.train()
    ...
    for idx, (input, target) in enumerate(train_loader):
        ...
        optimizer_underlying_model.zero_grad()
        optimizer_JPEG_Layer.zero_grad()
        loss.backward()
        optimizer_underlying_model.step()
...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    # set teacher as eval()
    if opt.JPEG_enable:
        module_list[-1].underlying_model.eval()
    else:
        module_list[-1].eval()
    ...
    for idx, data in enumerate(train_loader):
        ...
        loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd_method
...
def validate_attack(val_loader, model, criterion, opt, atk):
    """validation"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for idx, (input, target) in enumerate(val_loader):
        ...
        adv_images = atk(input, target)

        input = adv_images.float()
        if torch.cuda.is_available():
            input = input.cuda()
            target = target.cuda()
