...
def main(args):
    ...
    if args.distributed and args.sync_bn:
        if args.JPEG_enable:
            model.underlying_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.underlying_model)
        else:
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--outter_block_size', type=int, default=32, help='Ouuter block size')
