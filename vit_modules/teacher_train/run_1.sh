...
for trial in 99; do
    for teacher in ${teacher_list[*]}; do
        for dataset in ${dataset_list[*]}; do
            echo  ${dataset}
            CUDA_VISIBLE_DEVICES=2,3 python3.8 -m torch.distributed.launch --master_port=25672 --nnodes=1 --nproc_per_node=2 main.py \
        ...
        for q_max in 5; do
            for JEPG_alpha in 5; do
                ...
                while (( $(awk -v start="$start_1" -v end="$end_1" 'BEGIN { if (start <= end) print 1; else print 0 }') )); do
                    ...
                    CUDA_VISIBLE_DEVICES=2,3 python3.8 -m torch.distributed.launch --master_port=25672 --nnodes=1 --nproc_per_node=2 main_JPEG.py \
                                                            --model ${teacher}  --warmup_epochs 100 --epochs 600 \
                                                            --batch_size 128 --lr 2e-3 --update_freq 1 --use_amp true \
                                                            --data_set ${dataset} --save_ckpt_freq 100 \
