...
def main(args):
    ...
    # args.resume = os.path.join(output_dir, 'checkpoint-299.pth')
    ...
    test_stats = evaluate(data_loader_val, model, device, use_amp=args.use_amp, dist_enable=False)
    print(f"Accuracy of the network on {len(dataset_val)} test images: {test_stats['acc1']:.5f}%")

    '''
    Here we add the model
    '''
    args.sensitvity_dir = "../Senstivity/Senstivity_{}/SenMap/".format(args.data_set)

    # Here we start to find the senstivity

    if args.distributed:
        data_loader_train.sampler.set_epoch(0)

    model.eval()
    args.Nexample = len(data_loader_train.dataset) - 50
    Y_sens, Cb_sens, Cr_sens = get_sens(data_loader_train, model, args, mean, std, save=True)
